...
void runIRContextTests() {
    ...
    assert(&context1 == &context2 &&
           "IRContext::getInstance() should return the same instance.");
    ...
    assert(void_ty == context1.getVoidType() &&
           "getVoidType should be cached.");
    assert(label_ty == context1.getLabelType() &&
           "getLabelType should be cached.");
    assert(int1_ty == context1.getInt1Type() &&
           "getInt1Type should be cached.");
    assert(int32_ty == context1.getInt32Type() &&
           "getInt32Type should be cached.");
    assert(float_ty == context1.getFloatType() &&
           "getFloatType should be cached.");
    ...
    assert(int32_ptr_ty == context1.getInt32PtrType() &&
           "getInt32PtrType should be cached.");
    assert(float_ptr_ty == context1.getFloatPtrType() &&
           "getFloatPtrType should be cached.");
    assert(int1_ptr_ty == context1.getPointerType(int1_ty) &&
           "getPointerType should be cached.");
    assert(int32_ptr_ty != int1_ptr_ty &&
           "Pointers to different types should be different.");
    ...
    std::cout << "Int32 Ptr Ptr Type: " << int32_ptr_ptr_ty->print()
              << std::endl;
    ...
    assert(array_ty1 == array_ty2 &&
           "getArrayType should be cached for same element type and size.");
    assert(array_ty1 != array_ty3 &&
           "Arrays of different sizes should be different types.");
    assert(array_ty1 != array_ty4 &&
           "Arrays of different element types should be different types.");
    ...
    assert(func_ty1 == func_ty2 &&
           "getFunctionType should be cached for same signature.");
    assert(func_ty1 != func_ty3 &&
           "Functions with different signatures should be different types.");
    assert(func_ty1 != func_ty4 &&
           "Functions with different return types should be different types.");
