...
#include <string>
/*
我又想吐槽了，由于C++没有像rust那样方便的enum sum type
所以需要些一个OpID的enum，然后写一个方便映射的INSTR_ID2STRING表
然后还需要各自的函数来处理，不解耦这块没得说的
*/

// 编译器后端中用于 IR 打印与调试的辅助工具指令 ID 到字符串的映射
static std::array<std::string, 38> const INSTR_ID2STRING{
    "ret",         "br",     "add",           "sub",    "mul",    "mul64",
    "sdiv",        "srem",   "fadd",          "fsub",   "fmul",   "fdiv",
    "alloca",      "load",   "store",         "memset", "icmp",   "fcmp",
    "phi",         "call",   "getelementptr", "and",    "or",     "xor",
    "ashr",        "shl",    "lshr",          "asr64",  "shl64",  "lsr64",
    "zext",        "fptosi", "sitofp",        "cmpbr",  "fcmpbr", "loadoffset",
    "storeoffset", "select",
};

// OpID2String: 将 OpID 枚举值转换为对应字符串。
...
// CMP2STR：用于将整数比较操作码（CmpOp 枚举）转换为对应的字符串。
// 映射关系：eq (==), ne (!=), sgt (>), sge (>=), slt (<), sle (<=)。
// 用于 ICmp 指令打印。
...
// FCMP2STR：用于将浮点比较操作码（CmpOp 枚举）转换为对应字符串。
// 映射关系：ueq (unordered equal), une (!=), ugt (>), uge (>=), ult (<), ule
// (<=)。 通常用于 FCmp 指令的打印。
...
/**
 * @brief 打印操作数的字符串表示（不包含类型）。
 *
 * 该函数根据操作数的实际类型（运行时类型）来生成对应的 IR 打印格式。
 *
 * - 若操作数是全局变量（GlobalVariable）或函数（Function），则前缀使用
 * '@'，如：@global_var
 * - 若是常量（Constant），直接调用其 print()
 * 方法输出常量字面量表示，如：42, 3.14, true
 * - 否则默认使用局部值（如变量、寄存器、临时值），前缀使用 '%'，如：%tmp1、%a
 *
 * @param v 指向 IR 中的 Value 对象指针
 * @return std::string 返回值的字符串形式（不包含类型信息）
 */
...
BinaryInst *BinaryInst::createAdd(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(v1->getType()->isPointerType() ? v1->getType()
                                                         : v2->getType(),
                          Instruction::OpID::add, v1, v2, bb);
}

BinaryInst *BinaryInst::createSub(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::sub, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createMul(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::mul, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createMul64(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::mul64, v1,
                          v2, bb);
}

BinaryInst *BinaryInst::createSDiv(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::sdiv, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createSRem(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::srem, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createFAdd(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getFloatType(), Instruction::OpID::fadd, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createFSub(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getFloatType(), Instruction::OpID::fsub, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createFMul(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getFloatType(), Instruction::OpID::fmul, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createFDiv(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getFloatType(), Instruction::OpID::fdiv, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createAnd(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::land, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createOr(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::lor, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createXor(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::lxor, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createAsr(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::asr, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createLsl(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::shl, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createLsr(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::lsr, v1, v2,
                          bb);
}

BinaryInst *BinaryInst::createAsr64(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::asr64, v1,
                          v2, bb);
}

BinaryInst *BinaryInst::createLsl64(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::shl64, v1,
                          v2, bb);
}

BinaryInst *BinaryInst::createLsr64(Value *v1, Value *v2, BasicBlock *bb) {
    return new BinaryInst(Type::getInt32Type(), Instruction::OpID::lsr64, v1,
                          v2, bb);
}
...
CmpInst::CmpInst(Type *ty, CmpOp op, Value *lhs, Value *rhs, BasicBlock *bb)
    : Instruction(ty, Instruction::OpID::cmp, 2, bb),
      cmp_op_(op) {
    setOperand(0, lhs);
    setOperand(1, rhs);
}
...
void CmpInst::negation() {
    switch (getCmpOp()) {
    case CmpOp::EQ: cmp_op_ = CmpOp::NE; break;
    case CmpOp::NE: cmp_op_ = CmpOp::EQ; break;
    case CmpOp::GT: cmp_op_ = CmpOp::LE; break;
    case CmpOp::GE: cmp_op_ = CmpOp::LT; break;
    case CmpOp::LT: cmp_op_ = CmpOp::GE; break;
    case CmpOp::LE: cmp_op_ = CmpOp::GT; break;
    default:        break;
    }
    return;
}
...
FCmpInst::FCmpInst(Type *ty, CmpOp op, Value *lhs, Value *rhs, BasicBlock *bb)
    : Instruction(ty, Instruction::OpID::fcmp, 2, bb),
      cmp_op_(op) {
    setOperand(0, lhs);
    setOperand(1, rhs);
}

FCmpInst *FCmpInst::createFCmp(CmpOp op, Value *lhs, Value *rhs,
                               BasicBlock *bb) {
...
GetElementPtrInst::GetElementPtrInst(Value *ptr, std::vector<Value *> idxs,
                                     BasicBlock *bb)
    : Instruction(PointerType::get(getElementType(ptr, idxs)),
                  Instruction::OpID::getelementptr, 1 + idxs.size(), bb) {
    setOperand(0, ptr);
...
std::string StoreInst::print() {
    return OpID2String(this->getInstrType()) + " " +
           this->getOperand(0)->getType()->print() + " " +
           printAsOp(this->getOperand(0)) + ", " +
...
AllocaInst::AllocaInst(Type *ty, BasicBlock *bb)
    : Instruction(PointerType::get(ty), Instruction::OpID::alloca, 0, bb),
      alloca_ty_(ty) {}
...
std::string FpToSiInst::print() {
    return "%" + this->getName() + " = " + OpID2String(this->getInstrType()) +
           " " + this->getOperand(0)->getType()->print() + " " +
           printAsOp(this->getOperand(0)) + " to " +
           this->getDestType()->print();
}
...
std::string PhiInst::print() {
    ...
    if (this->getNumOperands() / 2 <
        ...
        for (auto pre_bb: this->getParent()->getPreBasicBlocks()) {
            if (std::find(this->getOperands().begin(),
...
CmpBrInst::CmpBrInst(CmpOp op, Value *lhs, Value *rhs, BasicBlock *if_true,
                     BasicBlock *if_false, BasicBlock *bb)
    : Instruction(Type::getVoidType(), Instruction::OpID::cmpbr, 4, bb),
      cmp_op_(op) {
...
CmpBrInst::CmpBrInst(CmpOp op, Value *lhs, Value *rhs, BasicBlock *bb)
    : Instruction(Type::getVoidType(), Instruction::OpID::cmpbr, 4, bb),
      cmp_op_(op) {
    setOperand(0, lhs);
    setOperand(1, rhs);
...
CmpBrInst *CmpBrInst::createCmpBr(CmpOp op, Value *lhs, Value *rhs,
...
FCmpBrInst::FCmpBrInst(CmpOp op, Value *lhs, Value *rhs, BasicBlock *if_true,
                       BasicBlock *if_false, BasicBlock *bb)
    : Instruction(Type::getVoidType(), Instruction::OpID::fcmpbr, 4, bb),
      cmp_op_(op) {
    setOperand(2, if_true);
    setOperand(3, if_false);
    setOperand(0, lhs);
...
FCmpBrInst *FCmpBrInst::createFCmpBr(CmpOp op, Value *lhs, Value *rhs,
                                     BasicBlock *if_true, BasicBlock *if_false,
                                     BasicBlock *bb) {
    if_true->addPreBasicBlock(bb);
    if_false->addPreBasicBlock(bb);
...
LoadOffsetInst::LoadOffsetInst(Type *ty, Value *ptr, Value *offset,
                               BasicBlock *bb)
...
LoadOffsetInst *LoadOffsetInst::createLoadOffset(Type *ty, Value *ptr,
                                                 Value *offset,
                                                 BasicBlock *bb) {
...
StoreOffsetInst::StoreOffsetInst(Value *val, Value *ptr, Value *offset,
                                 BasicBlock *bb)
...
Type *StoreOffsetInst::getStoreType() const {
    return static_cast<PointerType *>(getOperand(1)->getType())
        ->getElementType();
}

StoreOffsetInst *StoreOffsetInst::createStoreOffset(Value *val, Value *ptr,
                                                    Value *offset,
                                                    BasicBlock *bb) {
...
SelectInst::SelectInst(Type *type, Value *cond, Value *true_val,
                       Value *false_val, BasicBlock *bb)
    : Instruction(type, Instruction::OpID::select, 3, bb) {
    setOperand(0, cond);
    setOperand(1, true_val);
    setOperand(2, false_val);
}

SelectInst *SelectInst::createSelect(Type *type, Value *cond, Value *true_val,
                                     Value *false_val, BasicBlock *bb) {
...
LoadImmInst::LoadImmInst(Type *type, Value *cons, BasicBlock *bb)
    : Instruction(type, Instruction::OpID::loadimm, 1, bb) {
    setOperand(0, cons);
}
...
LoadImmInst::LoadImmInst(Type *type, Value *cons)
    : Instruction(type, Instruction::OpID::loadimm, 1) {
    setOperand(0, cons);
}
...
std::string LoadImmInst::print() {
    ...
    std::string instr_ir =
        "%" + getName() + "= " +
        (this->getType()->isIntegerType() ? "add i32 0 ,"
                                          : "fadd float 0x0 ,") +
        printAsOp(getOperand(0));
    return instr_ir;
}
...
CastInst::CastInst(Type *type, Value *val, BasicBlock *bb)
    : Instruction(type, Instruction::OpID::bitcast, 1, bb),
      origin_type(val->getType()) {
    setOperand(0, val);
}

CastInst::CastInst(Type *type, Value *val)
    : Instruction(type, Instruction::OpID::bitcast, 1),
      origin_type(val->getType()) {
    setOperand(0, val);
}
...
std::string CastInst::print() {
    static int num = 0;
    std::string instr_ir =
        "%" + getName() + "= " + "bitcast " + origin_type->print() + " " +
        printAsOp(getOperand(0)) + " to " + getType()->print();
    return instr_ir;
}
