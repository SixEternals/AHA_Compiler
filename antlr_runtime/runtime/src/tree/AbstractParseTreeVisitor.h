...
public:
    /// The default implementation calls <seealso cref="ParseTree#accept"/> on
    /// the specified tree.
     ...
     * method to behave properly in respect to the specific algorithm in
     * use.</p>
     */
...
protected:
    ...
    /// either all children are visited or <seealso
    /// cref="#shouldVisitNextChild"/> returns
    ...
    /// <seealso cref="#visitChildren"/> will return the result of the last
    /// child visited (or return the initial value if the node has no children).
    /// </summary>
    /// <param name="aggregate"> The previous aggregate value. In the default
    /// implementation, the aggregate value is initialized to
    /// <seealso cref="#defaultResult"/>, which is passed as the {@code
    /// aggregate} argument to this method after the first child node is
    /// visited. </param> <param name="nextResult"> The result of the
    /// immediately preceeding call to visit a child node.
    /// </param>
    ...
    /// returned by a call to <seealso cref="#defaultResult"/>. This method is
    /// not called after the last child is visited. <p/> The default
    /// implementation always returns {@code true}, indicating that
    /// {@code visitChildren} should only return after all children are visited.
    /// One reason to override this method is to provide a "short circuit"
    /// evaluation option for situations where the result of visiting a single
    /// child has the potential to determine the result of the visit operation
    /// as a whole.
    /// </summary>
    /// <param name="node"> The <seealso cref="ParseTree"/> whose children are
    /// currently being visited. </param> <param name="currentResult"> The
    /// current aggregate result of the children visited to the current point.
    /// </param>
    /// <returns> {@code true} to continue visiting children. Otherwise return
    /// {@code false} to stop visiting children and immediately return the
    /// current aggregate result from <seealso cref="#visitChildren"/>.
    /// </returns>
    virtual bool shouldVisitNextChild(ParseTree * /*node*/,
                                      std::any const & /*currentResult*/) {
        return true;
    }
};
